...
const ExperimentCard = ({ experiment }) => {
  ...
  return (
    <motion.div whileHover={{ scale: 1.03, boxShadow: "0 8px 24px rgba(0, 0, 0, 0.12)", borderColor: "rgba(99, 102, 241, 0.5)" }} transition={{ type: "spring", stiffness: 300, damping: 15 }} className="bg-white p-6 rounded-3xl shadow-soft border border-transparent cursor-pointer flex flex-col h-full">
      <div className="flex items-start justify-between">
        ...
        <span className={`text-sm font-semibold px-3 py-1.5 rounded-full border ${statusColors[experiment.statusKey]}`}>
          {t(experiment.statusKey)}
        </span>
      </div>
      <div className="mt-4 flex-grow">
        <h3 className="font-bold text-slate-800 text-xl leading-snug">{experiment.name}</h3>
        <p className="text-sm text-slate-500 mt-2">{experiment.resultKey}</p>
      </div>
